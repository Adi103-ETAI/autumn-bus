...
import { AutumnBusAgentSession } from '../dist/index.js'
...
const healthy = { name: 'autumn-bus', protocolVersion: '0.1', status: 'ready', features: ['session-retirement'] }
...
test('failed initial heartbeat retires registration without hiding the startup error', async (t) => {
  ...
  await assert.rejects(AutumnBusAgentSession.start(options), /initial heartbeat failed/)
...
test('close drains setState, is shared, and rejects further operations', async (t) => {
  ...
  const session = await AutumnBusAgentSession.start(options)
...
test('abort during registration retires once the committed result arrives', async (t) => {
  ...
  const starting = AutumnBusAgentSession.start({ ...options, signal: controller.signal })
...
test('heartbeat failure still attempts retirement and preserves the error', async (t) => {
  ...
  const session = await AutumnBusAgentSession.start({ ...options, heartbeatIntervalMs: 5 })
  t.after(() => session.close())
  await session.done
...
test('incompatible health fails before sending credentials or replacing an execution', async (t) => {
  for (const health of [{ ...healthy, features: undefined }, { ...healthy, protocolVersion: '0.2' }, { ...healthy, status: 'not_ready' }]) {
    ...
    await assert.rejects(AutumnBusAgentSession.start(options), error => error.code === 'CONFLICT')

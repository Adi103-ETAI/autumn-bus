...
| --- | --- | --- |
| F01: privileged credentials inherited by harness | Strip admin, standard scope, and configured scope-token variables before launching the child. | [Credential isolation](../cmd/autumn-bus/credential_isolation_test.go); existing managed-launch tests. |
...
- [#27](https://github.com/Adi103-ETAI/autumn-bus/issues/27): transaction authority, retirement and lifecycle fixes are implemented here; the broader host execution-authority reporting design still needs its own review.
- [#28](https://github.com/Adi103-ETAI/autumn-bus/issues/28): retirement, recovery, retention, errors/limits and corresponding schemas/tests were added to the protocol profile. Review the complete independent profile before declaring it stable.
- [#33](https://github.com/Adi103-ETAI/autumn-bus/issues/33), [#34](https://github.com/Adi103-ETAI/autumn-bus/issues/34), [#37](https://github.com/Adi103-ETAI/autumn-bus/issues/37): Cursor, OpenCode and Claude Code remain gates only if those adapters are promised at launch. This pass did not fabricate tested-version evidence.
- [#11](https://github.com/Adi103-ETAI/autumn-bus/issues/11), [#18](https://github.com/Adi103-ETAI/autumn-bus/issues/18): PostgreSQL and Supabase/shared deployment remain separate backend/deployment work, not prerequisites for a narrowly scoped local release.
- [#24](https://github.com/Adi103-ETAI/autumn-bus/issues/24), [#26](https://github.com/Adi103-ETAI/autumn-bus/issues/26): context extensions and active delivery remain design work outside the pull-only local launch scope.
- Adapter feasibility candidates and [#62](https://github.com/Adi103-ETAI/autumn-bus/issues/62) remain roadmap work. The original full issue inventory is preserved with the 2026-09-05 audit artifacts under the local ignored `_audit/production-2026-09-05/` directory; that snapshot is historical, not a fresh live issue count.

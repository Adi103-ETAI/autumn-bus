...
Autumn Bus ships a Go client in this module and a TypeScript client on npm. From `0.1.0-next.14`, the npm package also installs the matching native Go daemon and exposes the `autumn-bus` command. SDK imports do not load or start the daemon.
...
const outputs = new AutumnBusOutputClient(address, reader.credential)

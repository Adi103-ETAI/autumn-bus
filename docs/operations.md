...
Autumn Bus runs as one local daemon. The reference profile binds to `127.0.0.1` and stores accepted work in SQLite.
...
```bash
autumn-bus mcp stdio --scope my-project --agent reviewer
```
...
The argument-free form remains available inside `agent run`, which supplies `AUTUMN_BUS_ADDRESS` and `AUTUMN_BUS_AGENT_TOKEN`. Missing credentials/identity or mixed modes now fail. Independently launched editors should use configuration-only mode, not rely on a terminal's environment.
...
```sh
AUTUMN_BUS_ALLOWED_HOSTS=192.168.1.20:4765,bus.internal:4765 autumn-bus start
```

PowerShell uses the same setting:

```powershell
$env:AUTUMN_BUS_ALLOWED_HOSTS = "192.168.1.20:4765,bus.internal:4765"
autumn-bus start
```
...
The credential is read from `AUTUMN_BUS_AGENT_TOKEN`. The daemon address is
resolved from `--address`, then the `AUTUMN_BUS_ADDRESS` environment
...
The credential is read from `AUTUMN_BUS_SCOPE_TOKEN`. The daemon address is
resolved from `--address`, then `AUTUMN_BUS_ADDRESS`, then the local run file.
...
autumn-bus task add --title "Review checkout retries" --description "Check idempotency and error handling."
...
```bash
autumn-bus scope prune --before 2026-08-01T00:00:00Z
```
...
Inbound A2A work is limited independently for each remote principal. The defaults are 1,000 unfinished messages and 16 MiB of unfinished text. Set `AUTUMN_BUS_A2A_PRINCIPAL_MESSAGE_LIMIT` and `AUTUMN_BUS_A2A_PRINCIPAL_BYTE_LIMIT` before starting the daemon to choose stricter limits. The message limit must be from 1 through 9,999. The byte limit must be from 1 through 655,294,464.
...
```sh
AUTUMN_BUS_ALLOWED_ORIGINS=http://127.0.0.1:8080,https://dashboard.example autumn-bus start
```

PowerShell uses the same setting:

```powershell
$env:AUTUMN_BUS_ALLOWED_ORIGINS = "http://127.0.0.1:8080,https://dashboard.example"
autumn-bus start
```
...
```bash
autumn-bus scope export --id my-project --output my-project.bus.json
```
...
```bash
autumn-bus scope import --input my-project.bus.json
```
...
For a remote runtime, set `AUTUMN_BUS_ADMIN_TOKEN` and pass `--address`. Importing the exact same archive again is safe and does not duplicate state.

## Default paths

| Platform | Data directory | Runtime directory |
| --- | --- | --- |
| Windows | `%LOCALAPPDATA%\Autumn Bus` | Per-user temporary directory |
| macOS | `~/.local/share/autumn-bus` | Per-user temporary directory |
| Linux | `$XDG_DATA_HOME/autumn-bus` or `~/.local/share/autumn-bus` | `$XDG_RUNTIME_DIR/autumn-bus` or a per-user temporary directory |

Set `AUTUMN_BUS_DATA_DIR` and `AUTUMN_BUS_RUNTIME_DIR` to override these paths. The data directory contains `bus.db`. The runtime directory contains the current lock and credential-bearing run file.
...
```sh
autumn-bus backup --output /private/backup-location/bus-snapshot.db
```
...
```sh
autumn-bus scope delete --id my-project --confirm my-project
```

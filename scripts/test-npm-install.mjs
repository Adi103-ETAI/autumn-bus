...
const root = mkdtempSync(join(tmpdir(), 'autumn-npm-install-'))
const downloaded = []
...
try {
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'autumn-install-smoke', private: true, version: '1.0.0' }))
  ...
  const installed = JSON.parse(readFileSync(join(root, 'node_modules/@Adi103-ETAI/autumn-bus/package.json'), 'utf8'))
  ...
  await execute(process.execPath, ['--input-type=module', '-e', 'const m = await import("./node_modules/@Adi103-ETAI/autumn-bus/extensions/pi/index.mjs"); if (typeof m.default !== "function") process.exit(1)'], options)
  const mapPath = join(root, 'node_modules/@Adi103-ETAI/autumn-bus/dist/client.js.map')
  const sourceMap = JSON.parse(readFileSync(mapPath, 'utf8'))
  assert.equal(resolve(dirname(mapPath), sourceMap.sourceRoot, sourceMap.sources[0]), join(root, 'node_modules/@Adi103-ETAI/autumn-bus/src/client.ts'), 'Source maps must refer to the packaged source, not a temporary build directory')
  ...
  const launcher = join(root, 'node_modules/@Adi103-ETAI/autumn-bus/cli/autumn-bus.cjs')
  ...
  await execute(process.execPath, ['--input-type=module', '-e', 'import { AutumnBusClient, AutumnBusAgentSession } from "@Adi103-ETAI/autumn-bus"; if (typeof AutumnBusClient !== "function" || typeof AutumnBusAgentSession.start !== "function") process.exit(1)'], options)
  rmSync(join(root, 'node_modules', host.name), { recursive: true })
  await execute(process.execPath, ['--input-type=module', '-e', 'import { AutumnBusClient } from "@Adi103-ETAI/autumn-bus"; if (typeof AutumnBusClient !== "function") process.exit(1)'], options)

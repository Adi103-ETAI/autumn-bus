...
try {
  ...
  const installed = JSON.parse(readFileSync(join(root, 'node_modules/@autumn-dev/autumn-bus/package.json'), 'utf8'))
  ...
  await execute(process.execPath, ['--input-type=module', '-e', 'const m = await import("./node_modules/@autumn-dev/autumn-bus/extensions/pi/index.mjs"); if (typeof m.default !== "function") process.exit(1)'], options)
  const mapPath = join(root, 'node_modules/@autumn-dev/autumn-bus/dist/client.js.map')
  ...
  await execute(process.execPath, ['--input-type=module', '-e', 'import { AutumnBusClient, AutumnBusAgentSession } from "@autumn-dev/autumn-bus"; if (typeof AutumnBusClient !== "function" || typeof AutumnBusAgentSession.start !== "function") process.exit(1)'], options)
  rmSync(join(root, 'node_modules', host.name), { recursive: true })
  await execute(process.execPath, ['--input-type=module', '-e', 'import { AutumnBusClient } from "@autumn-dev/autumn-bus"; if (typeof AutumnBusClient !== "function") process.exit(1)'], options)

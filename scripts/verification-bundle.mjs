...
const defaultSecretNames = ['AUTUMN_BUS_ADMIN_TOKEN', 'AUTUMN_BUS_SCOPE_TOKEN', 'AUTUMN_BUS_AGENT_TOKEN', 'GH_TOKEN', 'GITHUB_TOKEN', 'NPM_TOKEN', 'NODE_AUTH_TOKEN', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY']
...
export function createVerificationBundle({ metadataPath, logPath, outDir, redactEnv = [], env = process.env }) {
  ...
  const manifest = {
    schemaVersion: 1, kind: 'autumn-bus-verification-bundle', verificationStatus: 'unreviewed',
...
export function verifyVerificationBundle(directory) {
  ...
  check(bundle?.schemaVersion === 1 && bundle.kind === 'autumn-bus-verification-bundle' && bundle.verificationStatus === 'unreviewed' && bundle.redaction?.manualReviewRequired === true, 'Invalid unreviewed bundle manifest')

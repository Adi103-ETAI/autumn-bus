...
test('artifact checks reject stale source, wrong package metadata and altered bytes', t => {
  const directory = mkdtempSync(join(tmpdir(), 'autumn-artifact-test-'))
  t.after(() => rmSync(directory, { recursive: true }))
  const file = join(directory, 'package.tgz')
  writeFileSync(file, 'synthetic tarball')
  const expected = { name: 'package', version: '1.0.0-next.1', requiredPath: 'bin/autumn-bus', source: { commit: 'a', sourceDigest: 'b' } }

...
These are contributor-reported observations from [PR #97](https://github.com/Adi103-ETAI/autumn-bus/pull/97), not compatibility evidence. The adapter remains experimental, `testedVersions` remains empty, and this note is not included in `compatibility/registry.json`.
...
Delivery is pull-only and process reachability does not prove model readiness. [Issue #37](https://github.com/Adi103-ETAI/autumn-bus/issues/37) remains open for an authenticated, reproducible [runbook](../RUNBOOK.md) attempt and independent review.

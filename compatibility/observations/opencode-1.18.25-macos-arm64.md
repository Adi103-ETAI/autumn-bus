...
These are contributor-reported observations from [PR #98](https://github.com/Adi103-ETAI/autumn-bus/pull/98), not reviewed compatibility evidence. The adapter remains experimental and this note is not included in `compatibility/registry.json`.
...
Delivery is pull-only and process reachability does not prove model readiness. Interactive approval prompts, other platforms, and other versions remain unverified. [Issue #34](https://github.com/Adi103-ETAI/autumn-bus/issues/34) remains open for a reproducible log with exact versions, investigation of the acknowledgement failure, and independent [runbook](../RUNBOOK.md) review.

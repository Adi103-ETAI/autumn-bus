...
func TestAuditCredentialChild(t *testing.T) {
	...
	for _, name := range []string{"AUTUMN_BUS_ADMIN_TOKEN", "AUTUMN_BUS_SCOPE_TOKEN", "BUS_AUDIT_SCOPE_TOKEN"} {
		if os.Getenv(name) != "" {
...
func TestManagedLauncherUsesPrivateScopeCache(t *testing.T) {
	...
	t.Setenv("AUTUMN_BUS_ADDRESS", "http://127.0.0.1:1") // local mode ignores ambient routing
	...
	t.Setenv("AUTUMN_BUS_ADMIN_TOKEN", "synthetic-admin-marker")
	...
	t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", "synthetic-mixed-authority")
...
func TestAuditManagedChildStripsPrivilegedCredentials(t *testing.T) {
	...
	t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", "synthetic-privileged-scope-marker")
	t.Setenv("AUTUMN_BUS_ADMIN_TOKEN", "synthetic-admin-marker")
	t.Setenv("BUS_AUDIT_CHILD", "1")

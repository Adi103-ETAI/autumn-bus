...
func TestSetEnvironmentReplacesExistingValues(t *testing.T) {
	...
	if len(result) != 3 || result[0] != "PATH=/bin" || result[1] != "AUTUMN_BUS_AGENT_TOKEN=new" || result[2] != "AUTUMN_BUS_AGENT_ID=worker" {
...
func TestInspectReceiptRequiresAgentToken(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "AUTUMN_BUS_AGENT_TOKEN is required") {
...
func TestListAgentsRequiresScopeToken(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "AUTUMN_BUS_SCOPE_TOKEN is required") {
		t.Fatalf("expected token-required error, got %v", err)
	}
}

...
func TestMCPStdioHelper(t *testing.T) {
	if os.Getenv("AUTUMN_BUS_MCP_STDIO_TEST_HELPER") != "1" {
		return
	}
	var args []string
	if raw := os.Getenv("AUTUMN_BUS_MCP_STDIO_TEST_ARGS"); raw != "" {
...
func mcpBridgeCommand(address, token string, stderr io.Writer) *exec.Cmd {
	...
	command.Env = setEnvironment(removeEnvironment(os.Environ(),
		"AUTUMN_BUS_ADDRESS",
		"AUTUMN_BUS_AGENT_TOKEN",
		"AUTUMN_BUS_SCOPE_TOKEN",
		"AUTUMN_BUS_ADMIN_TOKEN",
	),
		"AUTUMN_BUS_MCP_STDIO_TEST_HELPER", "1",
		"AUTUMN_BUS_ADDRESS", address,
		"AUTUMN_BUS_AGENT_TOKEN", token,
	)
...
func TestMCPStdioBridgeRejectsUnavailableDaemon(t *testing.T) {
	...
	if err == nil || !strings.Contains(stderr.String(), "could not connect to Autumn Bus") {
		t.Fatalf("expected unavailable daemon error, got %v; stderr: %s", err, stderr.String())
	}
}

func TestAgentRunHelper(t *testing.T) {
	if os.Getenv("AUTUMN_BUS_TEST_HELPER") != "1" {
		return
	}
	...
	if os.Getenv("AUTUMN_BUS_SCOPE_TOKEN") != "" {
		os.Exit(3)
	}
}
...
func TestStopCommandUsesProtectedLocalEndpoint(t *testing.T) {
	root := t.TempDir()
	t.Setenv("AUTUMN_BUS_DATA_DIR", filepath.Join(root, "data"))
	t.Setenv("AUTUMN_BUS_RUNTIME_DIR", filepath.Join(root, "run"))
...
func TestRunAgentOwnsHeartbeatAndEnvironment(t *testing.T) {
	...
	requireNoError(t, err)
	t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", scope.ScopeToken)
	t.Setenv("AUTUMN_BUS_TEST_HELPER", "1")
...
func TestSetEnvironmentReplacesExistingValues(t *testing.T) {
	result := setEnvironment([]string{"PATH=/bin", "AUTUMN_BUS_AGENT_TOKEN=old"},
		"AUTUMN_BUS_AGENT_TOKEN", "new",
		"AUTUMN_BUS_AGENT_ID", "worker",
	)
	require(t, len(result) == 3 && result[0] == "PATH=/bin" && result[1] == "AUTUMN_BUS_AGENT_TOKEN=new" && result[2] == "AUTUMN_BUS_AGENT_ID=worker", "unexpected environment: %#v", result)
}

func TestRemoveEnvironmentRemovesCredentials(t *testing.T) {
	result := removeEnvironment([]string{"PATH=/bin", "AUTUMN_BUS_SCOPE_TOKEN=secret", "OTHER=value"}, "autumn_bus_scope_token")
...
func TestInspectReceiptRequiresAgentToken(t *testing.T) {
	t.Setenv("AUTUMN_BUS_AGENT_TOKEN", "")
	err := inspectReceipt([]string{"--address", "http://127.0.0.1:1", "msg-1"})
	require(t, err != nil && strings.Contains(err.Error(), "AUTUMN_BUS_AGENT_TOKEN is required"), "expected token-required error, got %v", err)
}
...
func TestInspectReceiptEndToEnd(t *testing.T) {
	...
	t.Setenv("AUTUMN_BUS_AGENT_TOKEN", senderToken)
...
func TestInspectReceiptRejectsUnauthorizedCaller(t *testing.T) {
	...
	t.Setenv("AUTUMN_BUS_AGENT_TOKEN", outsider.AgentToken)
...
func TestListAgentsRequiresScopeToken(t *testing.T) {
	t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", "")
	err := listAgents([]string{"--address", "http://127.0.0.1:1"})
	require(t, err != nil && strings.Contains(err.Error(), "AUTUMN_BUS_SCOPE_TOKEN is required"), "expected token-required error, got %v", err)
}

func TestListAgentsRejectsPositionalArguments(t *testing.T) {
	t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", "unused")
...
func TestListAgentsEndToEnd(t *testing.T) {
	...
	defer cleanup()
	t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", scopeToken)
...
func TestListAgentsRejectsInvalidOrAgentCredential(t *testing.T) {
	...
	for name, testCase := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			t.Setenv("AUTUMN_BUS_SCOPE_TOKEN", testCase.token)

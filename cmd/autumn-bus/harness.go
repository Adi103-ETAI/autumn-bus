...
import (
	...
	"github.com/Adi103-ETAI/autumn-bus/adapters"
...
func harness(args []string) error {
	...
	complete = true
	fmt.Printf("Created %s. Review and add only the Autumn Bus entry to %s. This is not harness certification.\n", *output, adapter.Configuration.Location)
	return nil
}
...
func doctorHarness(host, scope string, jsonOutput bool) error {
	...
	if executable := adapter.Configuration.Executable; executable != "" {
		...
		if err != nil {
			...
			command.Env = removeEnvironment(os.Environ(), "AUTUMN_BUS_ADMIN_TOKEN", "AUTUMN_BUS_SCOPE_TOKEN", "AUTUMN_BUS_AGENT_TOKEN")
...
func probeLocalBridge(ctx context.Context, scope string) (count int, resultErr error) {
	...
	command.Env = removeEnvironment(os.Environ(), "AUTUMN_BUS_ADDRESS", "AUTUMN_BUS_ADMIN_TOKEN", "AUTUMN_BUS_SCOPE_TOKEN", "AUTUMN_BUS_AGENT_TOKEN")
	command.Stderr = io.Discard
	client := mcp.NewClient(&mcp.Implementation{Name: "autumn-bus-doctor", Version: bus.Version}, nil)

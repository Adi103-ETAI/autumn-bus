...
Run `autumn-bus harness config omp --scope my-project --agent omp-reviewer` and review the result before adding the Autumn Bus entry to .omp/mcp.json. Never paste the raw placeholder template. See [shared setup](../README.md) for scope creation, linking and removal.

OMP can discover other hosts’ configurations; disable duplicate Autumn Bus entries to avoid replacing your own execution. The intended identity is can1357/oh-my-pi; confirm this matches issue #42.

...
Run `autumn-bus harness config kimchi --scope my-project --agent kimchi-reviewer`. Review the output and add only its Autumn Bus entry to `~/.config/kimchi/harness/mcp.json`, preserving existing settings. See [shared setup](../README.md) for scope creation, linking and removal.

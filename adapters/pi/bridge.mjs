// A deliberately narrow client for Autumn Bus's local, newline-delimited MCP
...
export async function openBridge(config) {
  if (!config || !isAbsolute(config.command ?? '') || !Array.isArray(config.args) ||
    ...
    throw new Error('Generate an absolute-path Pi configuration with autumn-bus harness config pi')
  }
  const env = Object.fromEntries(Object.entries(process.env).filter(([key]) =>
    !/^AUTUMN_BUS_(ADDRESS|ADMIN_TOKEN|SCOPE_TOKEN|AGENT_TOKEN)$/i.test(key)))
  ...
  child.on('error', () => { fail(new Error('Autumn Bus bridge failed to start')); resolveExit() })
  child.on('close', () => { fail(new Error('Autumn Bus bridge closed; check daemon, scope and duplicate agent IDs')); resolveExit() })
  child.stdin.on('error', () => fail(new Error('Autumn Bus bridge input closed')))
  const send = message => {
    if (failure) throw failure
    child.stdin.write(JSON.stringify(message) + '\n', error => { if (error) fail(new Error('Autumn Bus bridge write failed')) })
  }
  ...
  child.stdout.on('data', chunk => {
    ...
    if (Buffer.byteLength(buffer) > 16 * 1024 * 1024) {
      fail(new Error('Autumn Bus bridge exceeded the 16 MiB frame limit'))
      child.kill()
      return
    }
    for (;;) {
      ...
      try { message = JSON.parse(line) } catch {
        fail(new Error('Autumn Bus bridge sent invalid protocol data'))
        child.kill()
        return
      }
      if (!message || message.jsonrpc !== '2.0') {
        fail(new Error('Autumn Bus bridge sent invalid JSON-RPC'))
        child.kill()
        return
      }
      if (message.method) {
      ...
      if (operation) operation.finish(message.error ? new Error(`Autumn Bus MCP error ${message.error.code}: ${String(message.error.message).slice(0, 512)}`) : undefined, message.result)
    }
  })
  const request = (method, params, signal, timeoutMs = 45_000) => new Promise((resolve, reject) => {
    ...
    if (pending.size >= 64) return reject(new Error('Too many pending Autumn Bus requests'))
    const id = ++sequence
    ...
    const abort = () => {
      ...
      finish(signal?.reason ?? new Error('Autumn Bus request cancelled or timed out'))
    }
  ...
  try {
    const initialized = await request('initialize', { protocolVersion: '2025-03-26', capabilities: {}, clientInfo: { name: 'autumn-bus-pi', version: '0.2.0' } }, undefined, 10_000)
    if (initialized?.protocolVersion !== '2025-03-26' || !initialized.capabilities?.tools) throw new Error('Incompatible Autumn Bus MCP bridge')

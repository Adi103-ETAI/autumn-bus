...
Run `autumn-bus harness config devin --scope my-project --agent devin-reviewer` and review the result before adding the Autumn Bus entry to Devin Customize > MCPs > Add custom MCP (STDIO fields). Never paste the raw placeholder template. See [shared setup](../README.md) for scope creation, linking and removal.

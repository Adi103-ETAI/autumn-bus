...
Autumn Bus uses MCP Streamable HTTP as an agent-facing integration surface. MCP supplies the tool transport. Autumn Bus supplies the coordination semantics.
...
The reference CLI forwards this MCP surface over stdio. With `autumn-bus mcp stdio --scope <id> --agent <id>`, the bridge reads a protected local scope token and owns registration, heartbeat and retirement. The argument-free `mcp stdio` form instead requires `AUTUMN_BUS_ADDRESS` and an execution-bound `AUTUMN_BUS_AGENT_TOKEN` supplied by a managed launcher. Mixing modes fails; missing identity fails rather than exposing an empty tool list. Neither mode stores separate coordination state.
...
MCP tool approval remains controlled by the harness. Autumn Bus credentials do not bypass host permissions, and a Bus request is never equivalent to human approval.
